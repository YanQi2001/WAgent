...
def batch_get_feed_details(
    page: Page,
    feeds: list[dict],
    config: CommentLoadConfig | None = None,
) -> dict:
    """Fetch multiple feed details in a single browser session.

    Reuses the same Page object across navigations to avoid per-feed
    CDP connection overhead and reduce anti-scraping triggers.

    Args:
        page: Existing CDP page to reuse.
        feeds: List of dicts with "feedId" and "xsecToken" keys.
        config: Comment loading config (comments disabled by default for speed).

    Returns:
        {"results": {feed_id: detail_dict, ...}, "errors": [feed_id, ...]}
    """
    if config is None:
        config = CommentLoadConfig()
    ...
    for i, feed in enumerate(feeds):
        ...
        if not feed_id or not xsec_token:
            errors.append(feed_id or f"unknown_{i}")
            continue

        try:
            detail = get_feed_detail(page, feed_id, xsec_token, config=config)
            results[feed_id] = detail.to_dict()
            logger.info("batch %d/%d: OK  feed_id=%s", i + 1, len(feeds), feed_id)
        except (PageNotAccessibleError, NoFeedDetailError, RuntimeError) as e:
            logger.warning("batch %d/%d: FAIL feed_id=%s — %s", i + 1, len(feeds), feed_id, e)
    ...
    logger.info("batch done: %d success, %d errors out of %d", len(results), len(errors), len(feeds))
