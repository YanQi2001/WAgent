...
def cmd_batch_feed_detail(args: argparse.Namespace) -> None:
    """批量获取多个 Feed 详情（单浏览器会话）。"""
    import json as _json
    from xhs.feed_detail import batch_get_feed_details

    feeds = _json.loads(args.feeds_json)
    if not feeds:
        _output({"results": {}, "errors": []})
        return

    browser, page = _connect(args)
    try:
        result = batch_get_feed_details(page, feeds)
        _output(result)
    finally:
        browser.close_page(page)
        browser.close()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # batch-feed-detail
    sub = subparsers.add_parser("batch-feed-detail", help="批量获取 Feed 详情")
    sub.add_argument("--feeds-json", required=True, help='JSON array: [{"feedId": "xxx", "xsecToken": "yyy"}, ...]')
    sub.set_defaults(func=cmd_batch_feed_detail)
